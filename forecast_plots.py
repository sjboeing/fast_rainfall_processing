...
nat_labels=['Leeds','Sheff.','York','Hull','B\'burn','Kendal','M\'boro.','N\'castle']
...
yorks_domain=Plot_Domain((-2.6, 0.2, 53.25, 54.57),labels=yorks_labels,latlabels=yorks_latlabels,lonlabels=yorks_lonlabels)
nat_domain=Plot_Domain((-3.55, 0.2, 53.15, 55.73),labels=nat_labels,latlabels=nat_latlabels,lonlabels=nat_lonlabels)
plot_domain=nat_domain
tnorm = mpl.colors.Normalize(vmin=0.,vmax=24.)
...
def perc_max_accums(fcst_str,date,perc=95,period=180,radius=30,evl=False):
    ...
    if evl:
        ...
        f=MyPlot(5,4,num,plot_domain,date_str,fcst_str,figdir+'/'+fcst_str+r'_{}_max_T{}_accum'.format(perc,period),figdir,lbotcol=True,suptitle=True)
    else:
        num=19
        figdir = args.user+"/output_plots/forecasts/"+fcst_str[0:6]+"/"+fcst_str+"/"
        if os.path.isdir(figdir) == False:
            os.system("mkdir "+figdir)
        f=MyPlot(5,4,num,plot_domain,date_str,fcst_str,figdir+'/'+date_str+r'_{}_max_T{}_accum'.format(perc,period),figdir,lbotcol=False,suptitle=True)
    #f.process_topo()
    f.start_bg()
    f.finish_bg()
    ...
    if evl:
        ...
        f.add_blegend('Rainfall (mm); Fcst: %02dZ %02d/%02d/%04d; Valid: %02d/%02d/%04d'%(fcst_date.hour,fcst_date.day,fcst_date.month,fcst_date.year,date.day,date.month,date.year))
    else:
    ...
    f.add_suptitle('%02d/%02d/%04d %02dpp forecast max accumulation in T%s'%(date.day,date.month,date.year,perc,period))
...
def exact_max_accums(fcst_str,date,period=180,evl=False):
    ...
    if evl:
        ...
        if os.path.isdir(figdir) == False:
            os.system("mkdir "+figdir)
        f=MyPlot(5,4,num,plot_domain,date_str,fcst_str,figdir+"/"+fcst_str+'_exact_max_T%s_accum'%(period),figdir,suptitle=True)
    else:
        num=18
        figdir=args.user+"/output_plots/forecasts/"+fcst_str[0:6]+"/"+fcst_str+"/"
        if os.path.isdir(figdir) == False:
            os.system("mkdir "+figdir)
        f=MyPlot(5,4,num,plot_domain,date_str,fcst_str,figdir+date_str+'_exact_max_T%s_accum'%(period),figdir,suptitle=True)    
    #f.process_topo()
    f.start_bg()
    f.finish_bg()
    #os.system('cp '+args.user+'/output_plots/background_tiles/5_4_%s_ctopo.pdf '%(num)+f.file_str+'ctopo.pdf')
    ...
    if evl:
        f.append_subtitle(0,mya.next()+r') radar OBS')
    f.add_legend('Rainfall (mm)\n\nForecast:\n%02dZ %02d/%02d/%04d\nValid:\n%02d/%02d/%04d'%(fcst_date.hour,fcst_date.day,fcst_date.month,fcst_date.year,date.day,date.month,date.year))
    f.add_suptitle('%02d/%02d/%04d exact forecast max accumulation in T%s'%(date.day,date.month,date.year,period))
    for ii in range(0,18):
...
def exact_day_accums(fcst_str,date,period=180,evl=False):
    ...
    if evl:
        ...
        if os.path.isdir(figdir) == False:
            os.system("mkdir "+figdir)
    f=MyPlot(5,4,num,plot_domain,date_str,fcst_str,figdir+'/'+date_str+'_exact_day_accum',figdir,suptitle=True)
    #f.process_topo()
    f.start_bg()
    f.finish_bg()
    #os.system('cp '+args.user+'/output_plots/background_tiles/5_4_%s_ctopo.pdf '%(num)+f.file_str+'ctopo.pdf')
...
def exact_accum_time(fcst_str,date,period=180,evl=False):
    ...
    f=MyPlot(5,4,num,plot_domain,date_str,fcst_str,figdir+date_str+'_exact_T{}_time'.format(period),figdir,norm=tnorm,cmap=listed_ccmap,levels=[],suptitle=True)
    #f.process_topo()
    f.start_bg()
    f.finish_bg()
    #os.system('cp '+args.user+'/output_plots/background_tiles/5_4_%s_ctopo.pdf '%(num)+f.file_str+'ctopo.pdf')
...
if args.evl is not None:
    ...
    while i > 0:
        ...
        fcst_strs.append("%04d%02d%02d_%02d"%(fcst_date.year,fcst_date.month,fcst_date.day,fcst_date.hour))
