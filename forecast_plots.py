...
def plot_forecasts(date, radius=30):
    ...
    def perc_max_accums(perc,period):
        mya=alphater()
        f=MyPlot(5,4,19,plot_domain,date_str,fcst_str,figdir+'/'+date_str+'_{}_max_T{}_accum'.format(perc,period),figdir,suptitle=True)
        #f.process_topo()
        #f.start_bg()
        #f.finish_bg()
        os.system('cp output_plots/background_tiles/5_4_19_ctopo.pdf '+f.file_str+'ctopo.pdf')
        os.system('cp output_plots/background_tiles/5_4_19_clines.pdf '+f.file_str+'clines.pdf')
        f.setup_decorations()
        ...
        f.add_suptitle('%02d/%02d/%04d %02dpp, r%r forecast max accumulation in T%s'%(date.day,date.month,date.year,perc,radius,period))
    ...
    def exact_max_accums(period):
        mya=alphater()
        f=MyPlot(5,4,18,plot_domain,date_str,fcst_str,figdir+'/'+date_str+'_exact_max_T{}_accum'.format(period),figdir,suptitle=True)
        #f.process_topo()
        #f.start_bg()
        #f.finish_bg()
