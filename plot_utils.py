...
class MultiLineHandler(HandlerBase):
    def create_artists(self, legend, orig_handle,
        ...
        for line_index in range(12):
            legend_lines.append(plt.Line2D([x0+0.37*width*(line_index//4),x0+(0.26+0.37*(line_index//4))*width], [0.33*(line_index%4)*height,0.33*(line_index%4)*height],
...
def scale_bar(ax, length=None, location=(0.75, 0.9), linewidth=3):
    ...
    #Plot the scalebar
    ax.plot(bar_xs, [sby, sby], transform=tmc, color='k', alpha=fudgealpha, linewidth=linewidth,solid_capstyle='butt')
    #Plot the scalebar label
    ax.text(sbx, sby, str(length) + ' km', transform=tmc,
            horizontalalignment='center', verticalalignment='bottom', weight=myweight,size=10.5, family=myfamily, alpha=fudgealpha)

# Plotting classes
class Lightmap(tiles.GoogleTiles):
    def _image_url(self, tile):
        x, y, z = tile
        url = '/gws/nopw/j04/icasp_swf/sboeing/tiles/toner/%s/%s/%s.png' % (
            z, x, y)
        return url
...
class Plot_Domain:
    ...
    def redraw_hillshademap(self,axis):
        for ii in [axis.get_images()[0]]:
             tempdata=ii._A[:]
             img=np.mean(tempdata[:,:,0:3],axis=2)/255.
    ...
    def redraw_linesmap(self,axis):
        for ii in [axis.get_images()[0]]:
             ...
             linesdata=np.dstack((rimg,rimg,rimg,1.0-2.*rimg/3.))
             ii.set_data(linesdata)
             ii.set_rasterized(True)

class MyPlot:
    def __init__(self,nx,ny,nt,plot_domain,date_str,fcst_str,file_str,dir_str,dpi=100,dpi2=400,awidth=2.2,aheight=2.2,atop=0.3,aright=0.1,amarg=0.3,armarg=None,almarg=None,abotcol=0.5,norm=norm,cmap=precip_colormap,levels=levels,lbotcol=False,suptitle=False):
        self.dpi=dpi
        self.dpi2=dpi2
        self.nx=nx
        self.ny=ny
    ...
    # Size of entire plot
    def my_size(self):
        self.xtot=self.nx*self.awidth+(self.nx-1)*self.aright+2*self.amarg #Use width rather than totwidth for last image
        if self.lbotcol and self.suptitle:
            self.ytot=self.ny*self.aheight+(self.ny-1)*self.atop+2*self.amarg+2*self.abotcol #Extra margin at base to accommodate legend
        elif self.suptitle:
            self.ytot=self.ny*self.aheight+(self.ny-1)*self.atop+2*self.amarg+self.abotcol #Extra margin on top to accommodate title
        elif self.lbotcol:
            self.ytot=self.ny*self.aheight+(self.ny-1)*self.atop+2*self.amarg+self.abotcol #Extra margin on top to accommodate title
        else:
            self.ytot=self.ny*self.aheight+(self.ny-1)*self.atop+2*self.amarg #Extra margin on top to accommodate title
    ...
    def makelocbot(self,factor=1.0,yfactor=1.0,yshift=0.0):
        width=((self.awidth*self.nx+self.aright*(self.nx-1))*factor)/self.xtot
        height=yfactor*self.abotcol/self.ytot
        lmargin=self.almarg/self.xtot
        bmargin=self.amarg/self.ytot+(0.5-0.5*yfactor)*self.abotcol/self.ytot+yshift/self.ytot
        left=lmargin
        bottom=bmargin
        return [left,bottom,width,height]
    ...
    def process_topo(self,title=''):
        ...
        for ii in range(self.nt):
            self.ax.append(plt.axes(self.makeloc(ii),projection=self.plot_domain.linesmap.crs))
    ...
    def add_legend(self,leg_string):
        ...
        t1=plt.text(1.0, 0.9, leg_string,
                 verticalalignment='top', horizontalalignment='right',
                 weight=myweight,family=myfamily,
                 size=10,alpha=fudgealpha)
        plt.text(1.0, 0.1, u'Toner map by Stamen \n Design, under CC BY 3.0. \n Data by OpenStreetMap, \n under ODbL.',
                 verticalalignment='bottom', horizontalalignment='right',
                 weight=myweight,family=myfamily,
                 bbox=dict(facecolor='1.0', boxstyle='round'),size=6,alpha=fudgealpha)
        cb_ax = plt.axes(self.makeloc(self.nt,factor=0.15))
        if(self.levels==[]):
            cb2 = matplotlib.colorbar.ColorbarBase(cb_ax, cmap=self.cmap,
                                        norm=self.norm)            
        else:
            cb2 = matplotlib.colorbar.ColorbarBase(cb_ax, cmap=self.cmap,
                                        norm=self.norm,
    ...
    def add_blegend(self,leg_string,lnarrow=False):
        ...
        tx_ax.get_yaxis().set_visible(False)
        if(lnarrow):
            t1=plt.text(0.0, 0.0, leg_string,
            ...
            t1=plt.text(0.58, 0.2, leg_string,
                 verticalalignment='bottom', horizontalalignment='left',
                 weight=myweight,family=myfamily,
                 size=10,alpha=fudgealpha)
            ...
            #     weight=myweight,family=myfamily,
            #     bbox=dict(facecolor='1.0', boxstyle='round'),size=8,alpha=fudgealpha)
            cb_ax = plt.axes(self.makelocbot(factor=0.55,yfactor=0.5),)
        if(self.levels==[]):
            cb2 = matplotlib.colorbar.ColorbarBase(cb_ax, cmap=self.cmap2,
                                        norm=self.norm, orientation='horizontal')            
        else:
            cb2 = matplotlib.colorbar.ColorbarBase(cb_ax, cmap=self.cmap2,
                                        norm=self.norm,
                                        boundaries=self.levels,
                                        ticks=self.levels,orientation='horizontal')
    ...
    def append_image(self,ii,cube):
        ...
        self.im.append(iplt.pcolormesh(cube,norm=self.norm,cmap=self.cmap,zorder=1,rasterized=True))
...
\usepackage[paperwidth="""+str(self.size[0])+r"""in,paperheight="""+str(self.size[1])+r"""in,top=0mm, bottom=0mm, left=0mm, right=0mm]{geometry}
\usepackage{graphicx}
\usepackage{fontenc}
\usepackage{overpic}
\begin{document}
...
""")            
    ...
    def cleanup(self):
        ...
        #os.system('pdfsizeopt --use-pngout=no '+self.file_str+'.pdf')                 
        plt.close('all') 
